...
void erode_workload(uint8_t *src, uint8_t *dst, uint8_t *kernel, int beg_r,
										int end_r, Config config) {
...
void erode_slave() {
	while (true) {
		...
		MPI_Recv(buffer.get(), src_area, MPI_UINT8_T, 0, 1, MPI_COMM_WORLD,
						 MPI_STATUS_IGNORE);

		erode_workload(buffer.get(), dst_buffer.get(), kernel.get(), beg, end,
									 config);
		MPI_Gatherv(dst_buffer.get(), dst_area, MPI_INT8_T, nullptr, nullptr,
								nullptr, MPI_UINT8_T, 0, MPI_COMM_WORLD);
	}
}
...
void erode_mpi(Mat &src, Mat &dst, Mat &kernel) {
	...
	MPI_Bcast(&config, 5, MPI_INT, 0, MPI_COMM_WORLD);
	MPI_Bcast(kernel.data, config.kernel_rows * config.kernel_cols, MPI_UINT8_T,
						0, MPI_COMM_WORLD);
	...
	for (int tid = 1; tid < concurrency; ++tid) {
		...
		MPI_Isend(src.data + displs.back(), (end - beg) * cols, MPI_UINT8_T, tid, 1,
